...
import { Heart, ShoppingBag, MapPin, Calendar, CheckCircle, AlertTriangle, Plus, CalendarCheck } from 'lucide-react';
...
const Home = () => {
    ...
    // avatarUrl kept for potential future use


    useEffect(() => {
        const loadEvents = async () => {
            try {
                const { data: list, error } = await supabase.from('events').select('id, title, event_date, event_time, location, image_url, creator_name, creator_avatar_url, created_at').order('created_at', { ascending: false }).limit(20);
                let attendeeIds = [];
                if (user) {
    ...
    const ACTION_ITEMS = [
        { path: '/adoption', bg: '#ffeaba', icon: <Heart size={28} color="var(--color-primary)" fill="var(--color-primary)" />, label: 'Adopción' },
        { path: '/alerts', bg: '#ffebee', icon: <AlertTriangle size={28} color="#e53935" />, label: 'Alertas' },
        { path: '/create-event', bg: '#ccfadd', icon: <Plus size={28} color="var(--color-social)" />, label: 'Crear Evento' },
    ...
    const eventCards = events.map((event) => (
        <div key={event.id} style={styles.eventCard} onClick={() => navigate(`/events/${event.id}`)}>
            <div style={styles.eventHeader}>
                <img src={getAvatarUrl(event.creator_avatar_url || event.creatorAvatar)} alt="" style={styles.creatorAvatar} loading="lazy" />
                <span style={styles.creatorName}>{event.creator_name || event.creatorName} creó un evento</span>
            </div>
            <img src={event.image_url || event.image || 'https://images.unsplash.com/photo-1587300003388-59208cc962cb?w=400&q=80'} alt={event.title} style={styles.eventImage} loading="lazy" />
            <div style={styles.eventDetails}>
                <h4 style={styles.eventTitle}>{event.title}</h4>
                <div style={styles.eventMeta}>
                    <div style={styles.metaItem}>
                        <Calendar size={14} />
                        <span>{formatEventDate(event.event_date || event.date)} • {formatTimeSlot(event.event_time || event.time)} hrs</span>
                    </div>
                    <div style={styles.metaItem}>
                        <MapPin size={14} />
                ...
                >
                    {event.attending ? <><CheckCircle size={18} /> Asistiré</> : 'Inscribirme'}
                </button>
            </div>
        </div>
    ));
    ...
    return (
        <div style={styles.container} className="fade-in">
            ...
            {isMobile ? (
                /* ── Mobile: quick actions row + events feed ── */
                <>
                    ...
                    <div style={styles.feedHeader}>
                        ...
                        <button type="button" style={styles.createEventBtn} onClick={() => navigate('/create-event')}>
                            Crear evento
                        </button>
                    </div>
                    ...
                    {eventsError ? (
                        ...
                        <div style={styles.eventsList}>{eventCards}</div>
                    )}
                </>
            ) : (
...
const styles = {
    ...
    eventsLoading: { color: 'var(--color-text-light)', margin: 0, padding: '0.5rem 0' },
    eventsList: {
        display: 'flex',
        flexDirection: 'column',
        gap: '1rem',
